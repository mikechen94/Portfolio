...
class PublicVendorParserSuite extends FunSuite {
  ...
  test("[additionForBadString] vendor parser should parse a program string correctly") {
    val insts = vp.parseString("bad\niconst 4\niconst 5\niadd\nprint\nfinish")
    assert(insts.length == 4)
  }

  test("[additionFor_p04-bad] vendor parser should parse a program file correctly") {
    val insts = vp.parse("programs/p04-bad-program.vm")
    assert(insts.length == 2)

    val all = Vector("iconst", "iconst")
    for (i <- insts.indices) {
      assert(insts(i).name == all(i))
    }
  }
